...
class BaseHMM(object):
        ...
        for t in range(1, n_samples):
            for i in range(self.n_states):
                for j in range(self.n_states):
                    log_A = log_mask_zero(self.A(obs_seq[t][2]))
    ...
    def _calc_beta(self, obs_seq, B_map):
        ...
        for t in range(n_samples - 2, -1, -1):
            for i in range(self.n_states):
                for j in range(self.n_states):
                    log_A = log_mask_zero(self.A(obs_seq[t][3]))
    ...
    def _update_model(self, new_model):
        ...
        """
        if 's' in self.tr_params:
            self.pi = (1 - self.learning_rate) * new_model['pi'] + self.learning_rate * self.pi
